...
public class AccountRepository {
    ...
    private static final Class<Account[]> serializationClass = Account[].class;

    public static Account load(String key) {
        Account value = getValues().get(key.toLowerCase());
        if (value == null) {
    ...
    public static void save(Account entity) {
        Map<String, Account> values = getValues();
        values.replace(entity.getKey().toLowerCase(), entity);
        Collection<Account> newValues = values.values();
        saveInternal(newValues);
    }

    private static void saveInternal(Collection<Account> newValues) {
    ...
    private static Map<String, Account> getValues() {
        try (FileReader fileReader = new FileReader(fileName); BufferedReader bufferedReader = new BufferedReader(fileReader)) {
            List<Account> list = bufferedReader.lines()
